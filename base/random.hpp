...
public:
    ...
    template<typename T> inline T pickOne(T begin, T end);
    template<typename T> inline T pickOne(T *container, size_t size);
    template<typename T> inline const T &choose(const T &a, const T &b);
};
