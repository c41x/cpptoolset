...
template <typename T_GENERATOR> template<typename T> T rng<T_GENERATOR>::pickOne(T begin, T end) { return begin + uint(std::distance(begin, end) - 1); }
template <typename T_GENERATOR> template<typename T> T rng<T_GENERATOR>::pickOne(T *container, size_t size) { return container[uint(size) - 1]; }
template <typename T_GENERATOR> template<typename T> const T &rng<T_GENERATOR>::choose(const T &a, const T &b) { if(boolean()) return b; return a; }
